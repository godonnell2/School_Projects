...
int skip_cd_cmd(const char *input)
{
    ...
    while (input[i] == ' ' || input[i] == '\t')
        i++;
    if (input[i] == 'c' && input[i + 1] == 'd' && (input[i + 2] == '\0' || input[i + 2] == ' ' || input[i + 2] == '\t'))
    {
        i += 2;
        while (input[i] == ' ' || input[i] == '\t')
            i++;
...
nheritance: When a process creates a child process (using fork), the child process inherits a copy of the parent's environment. However, by default, not all variables from the parent process are passed to the child.
