...
void	init_executor(t_executor *ex, t_command *cmds, int count, char **env)
{
	ex->cmd_i = 0;
	ex->cmd_count = count;
	ex->commands = cmds;
	ex->env_array = env;
	ex->prev_pipe_fd = -1;
	ex->pids = malloc(sizeof(pid_t) * count);
}

void	close_unused_fds(t_executor *ex)
{
	if (ex->cmd_i > 0)
...
int	needs_pipe(t_executor *ex)
{
	return (ex->cmd_i < ex->cmd_count - 1);
}

void	create_pipe(t_executor *ex)
{
...
int	fork_and_run_child(t_executor *ex)
{
	pid_t	pid;
...
void	execute_child(t_executor *ex)
{
	t_command	*cmd;
...
void	handle_parent(t_executor *ex)
{
	if (ex->cmd_i > 0)
		close(ex->prev_pipe_fd);
	if (needs_pipe(ex))
...
void	cleanup_executor(t_executor *ex)
{
	free(ex->pids);
}


void	execute_pipes(t_command *cmds, int count, char **env)
{
	t_executor	ex;

	init_executor(&ex, cmds, count, env);
	while (ex.cmd_i < ex.cmd_count)
	{
		if (needs_pipe(&ex))
			create_pipe(&ex);
		if (fork_and_run_child(&ex) == 0)
			execute_child(&ex);
		handle_parent(&ex);
		ex.cmd_i++;
	}
