...
int	main(void)
{
	...
	{ // Child process
		...
		perror("Child: Exec failed");        // Only runs if exec fails
												// No,
		that 's what' s so weird about exec !Your code will not run after the call to exec.
	...
	{ // Parent process
		...
		waitpid(pid, NULL, 0);
		// I asked chatgpt, the null is a PTR to the exit code of the child,
...
fork() creates a new process,
	allowing the parent to keep running while the child executes the new program.
For pipelines or any multi-step operations,
	fork() is essential to retain control in the parent process while
executing commands in the children.

Key Points
The parent and child are separate processes after fork().
The parent continues running its own code,
	completely independent of what happens in the child.
The exec in the child replaces only the child process,
	so it does not affect the parent process.
The	waitpid(void) call in the parent ensures the parent waits for the child to finish before proceeding
*/