...
/*   Updated: 2025/01/28 19:03:12 by gro-donn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	*extract_z_values(t_map_point *map_array, int size)
{
	...
	z_values = malloc(size * sizeof(float));
	if (!z_values)
	{
...
void	populate_z_values(t_point3d *points, t_map_point *map_array, t_map *map)
{
	...
	for (int y = 0; y < map->rows; y++)
	{
		for (int x = 0; x < map->cols; x++)
		{
			...
			{
				points[index].z = 0;
					// Or you could set it to another default value if needed
			}
			else
			{
...
static t_point2d	*prepare_iso_points(t_map *map, t_map_point *map_array,
...
int	main(int argc, char **argv)
{
	...
	z_values = extract_z_values(app.map.map_array, app.map.rows * app.map.cols);
	if (!z_values)
	{
		free(buffer);
